...
interface UseTrackerDataReturn {
  ...
  setNote: (dateString: string, note: string) => void;
...
export const useTrackerData = (currentYear: number): UseTrackerDataReturn => {
  const [config, setConfig] = useState<AppConfig>(() => {
    try {
      ...
      const defaults: AppConfig = {
        ...
        viewMode: 'goal',
        theme: storedTheme || 'cyan'
      };
      return stored ? { ...defaults, ...JSON.parse(stored), theme: storedTheme || JSON.parse(stored).theme || 'cyan' } : defaults;
    } catch (e) {
      return {
        inputLabel: DEFAULT_INPUT_LABEL,
        outputLabel: DEFAULT_OUTPUT_LABEL,
  ...
  const toggleOutput = useCallback((dateString: string) => {
    setData((prev) => {
      const current = prev[dateString] || { hours: 0, output: false };
      const newOutput = !current.output;
      return {
        ...prev,
        [dateString]: {
          ...current,
          output: newOutput,
          note: newOutput ? current.note : undefined // Clear note if output is unchecked
        }
      };
    });
  }, []);

  const setNote = useCallback((dateString: string, note: string) => {
    setData((prev) => {
      const current = prev[dateString] || { hours: 0, output: false };
      return {
        ...prev,
        [dateString]: { ...current, note }
      };
    });
  }, []);
  ...
  const stats = useMemo(() => {
    ...
    const entries = Object.entries(data)
      ...
      .map(([_, entry]) => entry as DailyEntry);
  ...
  }, [data, config.dailyGoal, currentYear]);
