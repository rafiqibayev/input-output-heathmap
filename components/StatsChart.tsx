...
export const StatsChart: React.FC<StatsChartProps> = ({ data, theme, currentYear }) => {
  const [isMobile, setIsMobile] = useState(false);
  const [viewMode, setViewMode] = useState<'daily' | 'weekly' | 'build'>('daily');
  const themeHex = THEMES[theme]?.hex || THEMES.red.hex;
  ...
  }, [data, weeklyData, buildData, isMobile, viewMode, currentYear]);
       ...
       <div className="flex justify-between items-end mb-2 px-2">
         <div className="flex items-center gap-4">
             <h3 className="text-[10px] font-bold uppercase tracking-widest text-gray-400 dark:text-neutral-500">
               {isMobile ? (viewMode === 'weekly' ? '12 Week Sprint' : '30 Day Sprint') : `${currentYear} Trajectory`}
             </h3>
