...
interface GridProps {
  ...
  config: AppConfig;
  currentYear: number;
  onCellClick: (data: DayCellData, e: React.MouseEvent) => void;
...
const HorizontalGrid: React.FC<{
  ...
  onCellMouseLeave: () => void;
  forceDesktop?: boolean;
}> = ({ data, config, currentYear, onCellClick, onCellMouseEnter, onCellMouseLeave, forceDesktop }) => {
    ...
    <div className="w-full flex flex-col overflow-x-auto pb-4">
      <div className="flex w-full gap-[2px] min-w-max">
        {gridWeeks.map((week, weekIndex) => (
          <div key={weekIndex} className={`flex-1 flex flex-col gap-[2px] ${forceDesktop ? 'w-5' : 'w-4 lg:w-5'}`}>
            {week.map((dayData) => (
...
export const Grid: React.FC<GridProps> = ({ data, config, currentYear, onCellClick, forceDesktop = false }) => {
  ...
  return (
    <div className="w-full relative">
      ...
      <div className={forceDesktop ? "block" : "hidden xl:block"}>
        <HorizontalGrid
          data={data}
          config={config}
          currentYear={currentYear}
