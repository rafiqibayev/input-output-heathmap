...
interface DayCellProps {
  ...
  config: AppConfig;
  isToday: boolean;
  onClick: (data: DayCellData, e: React.MouseEvent) => void;
...
export const DayCell: React.FC<DayCellProps> = React.memo(({ data, config, isToday, onClick, onMouseEnter, onMouseLeave }) => {
  ...
  if (hasOutput) {
    ...
    customBgStyle = { backgroundColor: activeTheme.hex };
    bgClass = 'hover:opacity-80'; // Simple hover effect for custom bg
  ...
  return (
    <button
      onClick={(e) => isTargetYear && onClick(data, e)}
    ...
    >
      ...
      <span
        className={`${textSizeClass} leading-none select-none ${textClass} tracking-tighter relative z-10 pt-[1px]`}
        style={{ ...customTextStyle, lineHeight: '1' }}
      >
        {label}
      </span>
    </button>
